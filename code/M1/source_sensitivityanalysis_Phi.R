...
for (c in 1:C) {
  ...
  PlotCIbandwithDataseries(
    ...
    # nameCI1 = expression(paste("AR(1); fix ", rho, ",", sigma[epsilon])),
    # nameCI2 = expression(paste("AR(1); model ", rho, ",", sigma[epsilon])),
    # nameCI3 = expression(paste("AR(2); model ", rho[1], ",", rho[2], ",", sigma[epsilon])),
