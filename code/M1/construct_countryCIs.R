...
array.jtl                <- array(NA, c(C.adj, Tend, L))
dimnames(array.jtl)[[1]] <- name.c[c.adj]
dimnames(array.jtl)[[2]] <- floor(years.t)
...
for (j in 1:C.adj) {
  ...
  for (t in 1:Tend) {
    ...
    a.lt[, t] <- rep(red.info["a.c[135]", "X50.percentile"], L)
    ## adjustment factor ##
